...
export default function AgentCard({ agent, onClick, minimalist = false }: { agent: Agent, onClick?: () => void, minimalist?: boolean }) {
    ...
    // Use pledgedAmount for pre-launch agents, simulated calculation for launched
    const marketCap = isLaunched ? 420000 + (Number(agent.id) * 1500) : (Number(agent.pledgedAmount || 0) / 10 ** 18) * 600 // Approx BNB price
    ...
    return (
        <motion.div
            ...
            className="group cursor-pointer bg-[#0A0A0A] hover:bg-[#111] border-b border-white/5 p-4 transition-colors grid grid-cols-[auto_1fr] gap-4 items-start"
        >
            {/* Image (Square, Clean) */}
            <div className="w-14 h-14 sm:w-16 sm:h-16 bg-white/5 rounded-lg overflow-hidden shrink-0 relative border border-white/5">
                <img
                    src={image}
                    alt={agent.ticker}
                    className="w-full h-full object-cover"
                    onError={(e) => {
                        (e.target as HTMLImageElement).src = `https://api.dicebear.com/9.x/shapes/svg?seed=${agent.ticker}`
                    }}
                />
            </div>

            {/* Content Container - Use min-w-0 to enable truncation */}
            <div className="min-w-0 flex flex-col justify-between h-14 sm:h-16 w-full">
                ...
                <div className="flex justify-between items-center w-full gap-2">
                    <div className="flex items-baseline gap-2 min-w-0 pr-2">
                        ...
                        <span className="text-xs text-text-dim truncate shrink-0">${agent.ticker}</span>
                    </div>
                ...
                <div className="flex justify-between items-center text-xs mt-1 w-full relative">
                    <div className="flex items-center gap-1.5 text-text-dim min-w-0 max-w-[50%]">
                        ...
                        <span className="truncate">{agent.creator.slice(0, 6)}...</span>
                    </div>
                    {/* Market Cap - Aligned right */}
                    <div className="font-mono text-white whitespace-nowrap ml-auto bg-[#0A0A0A]/80 pl-2">
                        <span className="text-text-dim mr-1 text-[10px] uppercase">MC</span>
